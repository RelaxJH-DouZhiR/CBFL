...
LastEditTime: 2021-06-30 13:59:33
Description: 分类器分类用于前百分比数据
 ██████╗██╗      █████╗ ███████╗███████╗██╗███████╗██╗ ██████╗ █████╗ ████████╗██╗ ██████╗ ███╗   ██╗
██╔════╝██║     ██╔══██╗██╔════╝██╔════╝██║██╔════╝██║██╔════╝██╔══██╗╚══██╔══╝██║██╔═══██╗████╗  ██║
██║     ██║     ███████║███████╗███████╗██║█████╗  ██║██║     ███████║   ██║   ██║██║   ██║██╔██╗ ██║
██║     ██║     ██╔══██║╚════██║╚════██║██║██╔══╝  ██║██║     ██╔══██║   ██║   ██║██║   ██║██║╚██╗██║
...
def save_result_to_excel(project, resList):
    ...
    sheetHead = ["project", "classification", "sam", "filePercentage", "k", "TP", "TN", "FP", "FN", "pre", "recall", "f1"]
    ...
    for i in resList:
        ...
        #
        excelLine += 1
    excel.save(f"{FATHER_PATH}CBFL/dataset/result/filePercentage_{project}" + ".xlsx")
        ...
        for classificationFunc in ["tree", "bayes", "KNN", "randomForest", "SVC"]:
            ...
            for sampling in ["none", "smote", "over", "under"]:
                ...
                for percentage in percentageList:
                    resDict["filePercentage"] = deepcopy(percentage)
                    for k in KList:
                        ...
                        print(f"\033[1;35mclassificationFunc : {classificationFunc} sampling : {sampling} percentage : {percentage} k : {k} \033[0m")
