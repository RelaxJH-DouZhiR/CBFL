...
LastEditTime: 2021-07-05 21:50:29
...
def use_classification(TEST_PATH, TRAIN_PATH, classificationFunc, k_Id, percentage, sampling):
    # 测试集-start
...
# 保存结果-start
def save_predictRes(TEST_PATH, project, percentage, kFold, k, resultList):
    CSV_PATH = f"{FATHER_PATH}CBFL/dataset/allPredictResult/result-{project}-{percentage}-{kFold}-{k}.csv"
    csvLine = 0  # csv行
    saveList = []
    with open(TEST_PATH) as f:
        csvFile = csv.reader(f)
...
if __name__ == "__main__":
    ...
    for project in ['chart', 'lang', 'time', 'math', 'mockito']:
        ...
        for k_Id in range(k):
            TEST_PATH = f"{FATHER_PATH}CBFL/dataset/{project}/percentage_traintest/test-{project}-{percentage}-{k}-{k_Id}.csv"
            TRAIN_PATH = f"{FATHER_PATH}CBFL/dataset/{project}/percentage_traintest/train-{project}-{percentage}-{k}-{k_Id}.csv"
            predictRes = use_classification(TEST_PATH, TRAIN_PATH, classificationFunc, k_Id, percentage, sampling)
            save_predictRes(TEST_PATH, project, percentage, k, k_Id, predictRes)
        print(f"\033[1;35mproject : {project} classificationFunc : {classificationFunc} sampling : {sampling} percentage : {percentage} k : {k} \033[0m")
